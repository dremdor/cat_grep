...
flags = ['e', 'i', 'v', 'c', 'l', 'n', 'h', 's'] #'h', 's', 'f', 'o'

all_words = set()
word_pattern = re.compile(r'\w+')

for test_file in test_files:
    with open(os.path.join(test_files_dir, test_file) + ".txt", 'r') as f:
        content = f.read()
        ...
        for pattern in patterns:
            for grep_cmd, output_file in [(your_grep, "your_output.txt"), (original_grep, "original_output.txt")]:
                if 'f' in flags_combination:
                    pattern_file_path = os.path.join(pattern_files_dir, f"PATTERN-FILE_{patterns.index(pattern) + 1:03d}") + ".txt"
            ...
            if cmp("your_output.txt", "original_output.txt"):
                ...
                print(bcolors.WARNING + f"CMD: {cmd}\t" + bcolors.FAIL + f"TEST FAILED: {test_file} with flags -{''.join(flags_combination)} and pattern {pattern}" + bcolors.ENDC)
                #exit(1)
